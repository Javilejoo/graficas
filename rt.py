...
class Raytracer(object):
    ...
    def rtRayColor(self, intercept, rayDirection, recursion = 0):
        ...
        if material.matType == OPAQUE:
            ...
            for light in self.lights:
                if light.lightType == "Ambient":
                    ...
                    if light.lightType == "Directional":
                        ...
                        lightDir = ml.restar_vectores(light.point, intercept.point)

                        lightDir_norm = ml.norma_linalg(lightDir)
                        if lightDir_norm != 0:
            ...
            reflect = reflectVector(intercept.normal, rayDirection_negado )
            ...
            bias = ml.multiplicar_vector_por_escalar(intercept.normal , 0.001)
            ...
            if outside:
                reflectOrig = ml.sumar_vectores(intercept.point, bias)
            else:
                reflectOrig = ml.restar_vectores(intercept.point, bias)
            ...
            if not totalInternalReflection(intercept.normal, rayDirection, 1.0, material.ior):
                ...
                if outside:
                    refractOrig = ml.restar_vectores(intercept.point, bias)
                else:
                 refractOrig = ml.sumar_vectores(intercept.point, bias)
